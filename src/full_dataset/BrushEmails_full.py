...
def get_emails_from_db():
    ...
    body_list = edb.get_raw_bodies_with_id()
    return body_list

def update_email(email_id, body):
    edb.update_brushed_body_full(email_id, body)
    
def update_lines(email_id, body_lines):
    edb.update_brushed_lines_full(email_id, body_lines)
...
def store_one_liner(edb, email_id, text):
    edb.update_brushed_one_line_full(email_id, text)
...
#         print 'PARSING EMAIL: %d'%(email_id)
        ...
        body = ''.join(ch for ch in processed_body.lower().translate(None, '\n\t\r') if ch not in exclude_set)
        # Store in DB
...
def test():
    (email_id, email_body) = edb.get_email_full(7)  # @UnusedVariable
    processed_body = process_email(email_body)
    cat_lines = concat_paragraph_lines(processed_body)
